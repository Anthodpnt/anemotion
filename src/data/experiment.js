export default {
  video: {
    src: [`${import.meta.env.VITE_CDN_URL}/videos/heider-simmel-experiment.mp4`],
    title: 'The Experiment Video',
    caption: '"Animating Anthropomorphism: Giving Minds To Geometric Shapes." by Frietz Heider & Marianne Simmel, 1944',
  },
  header: {
    title: 'Impact of Motion on Emotion',
    subtitle: 'An Experiment Conducted in 1944',
  },
  definitions: [
    {
      text: 'This is the story of a man who planned to meet a girl. Unfortunately, she comes to the rendez-vous with another man. After each of the man tells the other to leave, a fight starts and, hesitating, to get away from it, the girl flees into the room. [...]',
      voice: [
        '/voices/heider-simmel-experiment-interpretation.ogg',
        '/voices/heider-simmel-experiment-interpretation.mp3',
      ],
      volume: 1,
      caption: 'The common interpretation',
      duration: 16,
    },
  ],
  statistics: {
    title: 'The Results',
    values: [
      {
        value: '35<sup>%</sup>',
        text: 'of the audience described<br /> the large triangle as<br /> <b>aggressive</b>',
      },
      {
        value: '75<sup>%</sup>',
        text: 'of the audience described<br /> the circle as<br /> <b>afraid</b>',
      },
      {
        value: '50<sup>%</sup>',
        text: 'of the audience described<br /> the small triangle as<br /> <b>heroic</b>',
      },
    ],
  },
}
